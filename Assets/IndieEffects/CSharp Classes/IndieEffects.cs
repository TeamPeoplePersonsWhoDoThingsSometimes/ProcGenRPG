...
public class IndieEffects : MonoBehaviour
{
    ...
    public void OnPreRender () 
    {
        ...
        {		
            ...
            if (capture && DNRequire) 
            {
                ...
                DepthCam.camera.pixelRect = new Rect(0,0,Screen.width,Screen.height);
                DepthCam.camera.Render();
				DNBuffer.ReadPixels(new Rect(camera.pixelRect.x, camera.pixelRect.y, Screen.width, Screen.height), 0, 0);
                DNBuffer.Apply();
            }
            if (capture) 
            {
                ...
                dom.camera.aspect = asp;
                dom.camera.pixelRect = new Rect(0, 0, Screen.width, Screen.height);
                dom.camera.Render();
                RT.ReadPixels(new Rect(camera.pixelRect.x, camera.pixelRect.y, Screen.width, Screen.height), 0, 0);
                RT.Apply();
            }
    ...
    {	
        ...
        {		
            capture = true;
            RT = new Texture2D(Screen.width, Screen.height, TextureFormat.RGB24, false);
            RT.wrapMode = TextureWrapMode.Clamp;
            DNBuffer = new Texture2D(Screen.width, Screen.height, TextureFormat.ARGB32, false);
